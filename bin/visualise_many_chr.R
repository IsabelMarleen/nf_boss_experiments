...
visualise_simulation <- function(ptol, nrow) {
  ...
  # get the ordering of the OTUs based on mean coverage
  otu_order <- cov %>%
  ...
  # convert time units to pseudominutes
  cov <- cov %>%
...
num_otu <- unb_cond%>%pull(otu)%>%unique()%>%length()
num_bc <- unb_cond%>%pull(bc)%>%unique()%>%length()
colours <- list()
...
for (a in 1:num_bc){
  ...
  colfunc <- colorRampPalette(c(dark, light))
  colvals <- colfunc(num_otu)
  colours <- append(colours, colvals)
}
colours  <- unlist(colours)
  
  unb_plot_seq <- ggplot(
      data=arrange(unb_cond, bc),
      mapping=aes(x=seq_time, y=unb_ratio)) +
    geom_line(aes(group=interaction(otu,bc),colour=interaction(otu,bc)),linewidth=0.1,alpha=0.7) +
    scale_color_manual(values=colours)+
    ylab("cum. rejection rate") +
    xlim(0, max(unb_cond$seq_time))+
    ylim(0,1)+
    guides(colour = guide_legend(override.aes = list(linewidth = 20)))
  # unb_plot
 ggsave(output, unb_plot_seq, w=4, h=15)

   unb_plot_frag <- ggplot(
      data=unb_cond,
      mapping=aes(x=total, y=unb_ratio)) +
    geom_line(aes(group=interaction(otu,bc),colour=interaction(otu,bc)),size=0.1,alpha=0.7)+
 ...
 ggsave(output, unb_plot_frag, w=4, h=7/3)
  ...
  nreads_seq <- ggplot(
      data=unb,
            mapping=aes(x=seq_time, y=total, linetype=cond, colour=interaction(otu,bc))) +
  ...
  nreads_noncum_seq <- ggplot(
      data=mutate(unb, total_rate = if_else(time == 0, NA, total_rate)),
            mapping=aes(x=seq_time, y=total_rate, linetype=cond, colour=interaction(otu,bc))) +
    geom_line(alpha=0.8, linewidth=0.1)+
    xlim(0, max(unb_cond$seq_time))+
    scale_color_manual(values=colours, guide = "none") +
    ylab("# fragments/min")+
  ...
  meanc_seq <- ggplot(
      data=cov,
            mapping=aes(x=seq_time, y=mean_coverage, linetype=cond, colour=interaction(otu,bc))) +
    geom_line(alpha=0.8, linewidth=0.1) +
    xlim(0, max(unb_cond$seq_time))+
  ...
  lowc_seq <- ggplot(
      data=cov,
            mapping=aes(x=seq_time, y=low_coverage_prop, linetype=cond, colour=interaction(otu,bc))) +
    geom_line(alpha=0.8, linewidth=0.1) +
    scale_color_manual(values=colours, guide = "none") +
    xlim(0, max(unb_cond$seq_time))+
  ...
  evn_seq <- ggplot(
      data=cov,
            mapping=aes(x=seq_time, y=evenness, linetype=cond, colour=interaction(otu,bc))) +
    geom_line(alpha=0.8, linewidth=0.1) +
    scale_color_manual(values=colours, guide = "none") +
    xlim(0, max(unb_cond$seq_time))+
    ylab("evenness")+
